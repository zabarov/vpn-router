...
Every new host starts with one tunnel `/32`, one exact test domain for proxy
sources, and a server-side rollback deadman.
...
| Selected suffix from tunnel and proxy sources | Uses the selected strict exit |
...
| Second tunnel client outside `/32` | Behavior unchanged in every state |
| All users of a proxy source | Policy applies consistently to the whole container |
...
- any out-of-scope tunnel client changes behavior;
- host or source-namespace default/SSH routes change;
- a source container ID or tunnel topology changes;
